...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/08 16:03:02 by aemilien          #+#    #+#             */
/*   Updated: 2019/12/08 16:03:02 by aemilien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <server/irc.h>

static int	irc_topic_check_command(
...
int			irc_topic(t_env *e, int cs, t_token *tokens)
{
	...
	if (tokens[2].addr)
	{
		if (e->channels[e->fds[cs].channel].chop != cs)
		{
			return (irc_err(e, cs, ERR_CHANOPRIVSNEEDED,
