...
{
    if (msg)
        return (cbuffer_putcmd(&fd->buf_write, "TOPIC %.*s :%s\x0D\x0A",
                               chan_len, chan, msg));
    return (
        cbuffer_putcmd(&fd->buf_write, "TOPIC %.*s\x0D\x0A", chan_len, chan));
}
...
int c2s_topic(t_env *e, t_token *tokens)
{
    ...
    _c2s_topic(e->self, tokens[1].addr ? tokens[1].addr : e->self->channelname,
