...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/14 12:37:45 by aemilien          #+#    #+#             */
/*   Updated: 2019/12/14 12:37:47 by aemilien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static const t_s2c_error	g_s2c_error[] = {
	...
	{ERR_NOT_CONNECTED, "You need to be logged in before any command.", NULL},
...
static int					prefix_error(char *error_msg)
{
	...
	memset(error_msg, 0, LOGSIZE + 1);
	if (is_tty == -1)
		is_tty = isatty(1);
	time2iso(t);
	return (sprintf(error_msg, is_tty ?
