...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/14 13:17:47 by aemilien          #+#    #+#             */
/*   Updated: 2019/12/14 13:19:27 by aemilien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	s2c_join_is_me(t_env *e, const char *user, size_t len)
{
	...
	sprintf(expected, "%s!%s@%s",
		e->self->nickname, e->self->username, e->self->host);
...
int			s2c_join(t_env *e, t_token *tokens)
{
	...
	sprintf(msg, "%.*s joined the channel.",
		(int)tokens[0].len, tokens[0].addr);
