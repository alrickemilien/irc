...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/14 12:23:33 by aemilien          #+#    #+#             */
/*   Updated: 2019/12/14 12:38:14 by aemilien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static GtkWidget	*ui_build_nick_msg_pair(const char *msg, int is_rtl)
{
	...
	gtk_box_set_homogeneous(GTK_BOX(c), FALSE);
	l = ui_util_new_chat_label(strchr(msg, ':') + 1, "chat-message");
	if (is_rtl)
	{
		nick = ui_extract_nick(msg, GTK_TEXT_DIR_RTL);
		gtk_label_set_xalign(GTK_LABEL(l), 1);
		gtk_label_set_xalign(GTK_LABEL(nick), 1);
		gtk_box_pack_end(GTK_BOX(c), GTK_WIDGET(nick), FALSE, FALSE, 0);
		gtk_box_pack_end(GTK_BOX(c), GTK_WIDGET(l), FALSE, FALSE, 0);
	}
	else
	{
		...
		gtk_box_pack_start(GTK_BOX(c), GTK_WIDGET(nick), FALSE, FALSE, 0);
		gtk_box_pack_start(GTK_BOX(c), GTK_WIDGET(l), FALSE, FALSE, 0);
	}
	return (c);
}

GtkWidget			*ui_new_chat_message(
		t_ui_panel *ui, const char *msg)
{
	int			is_rtl;
	GtkWidget	*container;
